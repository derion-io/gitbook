...
1. **Pre-pick.** Before the Helper runs, the only directional signal is the transactor's declared slippage floors, so the pool folds the *signs* of `dAMin` and `dBMin` into a single net exposure and optimistically picks the bound adverse to it: net long → MAX (long is dear), net short or neutral → MIN.
2. **Re-check.** After the trade is solved, the pool recomputes the net exposure from the *realized* deltas ($$E \propto dA/s_A - dB/s_B$$) and requires the bound actually used to be adverse to it — whenever the two oracle prices diverged. When TWAP and spot agree, the pick is irrelevant.
