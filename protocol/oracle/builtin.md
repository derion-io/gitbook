...
| 224–254 | reserved | |
| 192–223 | `WINDOW` | Uniswap v3: the TWAP averaging window in seconds. Chainlink: the staleness bound in seconds. |
...
Because `WINDOW` carries two different meanings, set it for whichever branch `DECIMALS` selects.

### Uniswap v3

Any pair on the network is ready for a Derion pool without extra infrastructure. The spot comes from the pair's current tick and the TWAP from its mean tick over `WINDOW`, both as square-root prices scaled to Q128. A longer window resists manipulation better, at the cost of more spread in fast markets.
