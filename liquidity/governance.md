...
A strategy is immutable by construction. Changing policy means writing a new blob and moving the votes behind it. Only blobs created through the Vault are registered; a hand-deployed byte-compatible blob can be neither voted for nor promoted, so every record the Vault acts on passed its checks at creation.
...
Vote capture costs real capital: a strict majority of the time-weighted locked VaultLP, held long enough to out-age the rest, all of it at NAV and at risk in the same book it steers. Shares that are not escrowed carry no weight, so the bar is set by how much of the supply is locked, not by the supply itself. Standard token-governance levers (a timelock, a quorum, a per-holder cap) can be layered on if a deployment warrants.
